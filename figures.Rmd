...
CreateGainlossPlots <- function(x, facet_row_n = 1, pvalpos = 100, ncs_unq = NULL, order_by_medians = TRUE) {
  ...
  plt <- ggplot(resdf_melt) +
    ...
    geom_text(data = lbldf, aes(x = 1.5, y = pvalpos, label = lbls), parse = TRUE) +
...
cairo_pdf(file = "plts_ready_pdf/figure_2.pdf", width = conv_unit(18, "cm", "inch"), height = conv_unit(26.14, "cm", "inch"))
plt_fig2
dev.off()
```
...
plt_fig3b <- ggplot(tcelldf_long) +
  ...
  scale_colour_brewer(type = "div", palette = "Set1") +
  annotate("text", x = 1.5, y = tcelldf$`CD25+ fraction - peptide pool 1` %>% max %>% add(0.8),
           label = PrettyPvalues(pval_fc_unstim_orig),
           hjust = 0.5, parse = TRUE, size = 7/.pt) +
  annotate("text", x = 2.5, y = tcelldf$`CD25+ fraction - peptide pool 2` %>% max %>% add(0.8),
