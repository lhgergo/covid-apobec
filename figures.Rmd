...
# Introduction
The following file contains all the codes necessary to generate the figures for the manuscript "C>U mutations generate immunogenic peptides in SARS-CoV-2" by Balogh et al. (2025).
...
# setting the number of rows to skip ----------
n_skip = 0
...
CreateForestPlot <- function(mdl_data, varlabels = NULL, show.OR = TRUE) {
  # creating forestmodel object, preparing data
  mdl_data <- mdl_data[!is.na(mdl_data$p.value), ]
  mdl_data$index <- 1:nrow(mdl_data)
  
  if(show.OR) {
    mdl_data$conf.low <- exp(mdl_data$conf.low); mdl_data$conf.low[is.na(mdl_data$conf.low)] <- 1
    mdl_data$conf.high <- exp(mdl_data$conf.high); mdl_data$conf.high[is.na(mdl_data$conf.high)] <- 1
    mdl_data$OR <- exp(mdl_data$estimate)
    
    mdl_data$OR_and_confints <- PrettyPvalues(mdl_data$OR, only_number = TRUE)
    mdl_data$OR_and_confints <- mdl_data$OR_and_confints %>% substr(1, nchar(.)-1) %>%
      paste0(", \" (\", ", format(mdl_data$conf.low, digits = 2), ",\"-\",",
             format(mdl_data$conf.high, digits = 2), ", \")\")")
    plt_xlabel <- "Odds Ratio (95% CI)"
    col_title <- "Odds Ratio (95% CI)"
    
  } else {
    mdl_data$OR <- mdl_data$estimate
    mdl_data$conf.low[is.na(mdl_data$conf.low)] <- 0
    mdl_data$conf.high[is.na(mdl_data$conf.high)] <- 0
    
    mdl_data$OR_and_confints <- PrettyPvalues(mdl_data$OR, only_number = TRUE)
    mdl_data$OR_and_confints <- mdl_data$OR_and_confints %>% substr(1, nchar(.)-1) %>%
      paste0(", \" (\", ", format(mdl_data$conf.low, digits = 2), ",\"-\",",
             format(mdl_data$conf.high, digits = 2), ", \")\")")
    plt_xlabel <- "Logistic model coefficient"
    col_title <- "Coefficient"
  }
  
  # adding labels if needed
  if(!is.null(varlabels)) mdl_data$term <- varlabels
  
  # coverting labels into factor
  mdl_data$term <- factor(mdl_data$term, levels = rev(mdl_data$term))
  
  # creating the ggplot object
  plt1 <- mdl_data %>% 
    ggplot(aes(x = OR, y = term)) +
    geom_vline(xintercept = 1, linetype = "dashed", size = 1, color = "#cecece") + 
    geom_point(size = 3, shape = 18, color = "black") + geom_errorbar(aes(xmin =  conf.low, xmax = conf.high), width = 0.2) +
    scale_x_continuous(limits = c(0, max(mdl_data$conf.high)), breaks = seq(0.25, max(mdl_data$conf.high), by = 0.5)) +
    xlab(plt_xlabel) + 
    thm +
  ...
  table_base <- ggplot(mdl_data, aes(y=label)) +
    ylab(NULL) + xlab("  ") + 
  ...
  tab1 <- table_base +
    labs(title = "space") +
    geom_text(aes(y = rev(index), x = 1, label = OR_and_confints), parse = TRUE, size = 7/.pt) +
    ggtitle(col_title)
  
  tab2 <- table_base +
    geom_text(aes(y = rev(index), x = 1, label = PrettyPvalues(p.value, only_number = TRUE)), parse = TRUE, size = 7/.pt) +
    ggtitle("P")
  
  align_plots(plt1, tab1, tab2, align = "h") %>% plot_grid(plotlist = ., nrow = 1, rel_widths = c(0.65, 0.175, 0.175))
}
...
```{r}
# Figure 1A ----------
freqsdf  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 1", skip = n_skip)
...
freqsdf_quaterly  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 2", skip = n_skip)
dunnpdf  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 3", skip = n_skip)
...
plt_fig1b <- ggplot(freqsdf_quaterly %>% filter(`Nucleotide change` %in% names(frequent_ncs)),
  ...
  geom_label_repel(data = labeldf, mapping = aes(x = "24 Q3", y = `Mean mutation count`, label = `Nucleotide change`, color = `Nucleotide change`),
                   direction = "y", seed = 1234, nudge_x = 4, min.segment.length = 0, size = 7/.pt) +
...
# Figure 1C ----------
freqsdf_traj  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 4", skip = n_skip)
...
# Figure 1D ----------
gainlossdf  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 5", skip = n_skip)
...
# Figure 1E ----------
aachfreqsdf  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 7", skip = n_skip)
cutoff_th = 0.05
...
# Figure 1F ----------
aaprefsdf  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 6", skip = n_skip)
...
plt_fig1f <- aaprefsdf %>% 
  ggplot() + 
  geom_bar(aes(x = `Amino acid`, y = `Median bit score`), stat = "identity") +
  annotate("text", y = 0.6, x = "E", label = corr_rho, hjust = 0, size = 7/.pt) +
  annotate("text", y = 0.5, x = "E", label = corr_pval, hjust = 0, size = 7/.pt, parse = "TRUE") +
  ylab("Median sum bitscores") +
  xlab("Amino acid") +
  thm
...
# Figure 1H ----------
gainlossdf_aach  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 8", skip = n_skip)
...
# assembling Figure 1 ----------
row1 <- plot_grid(plt_fig1a, plt_fig1b, labels = c("A", "B"), label_size = 7)
row2 <- plot_grid(plt_fig1c, labels = "C", label_size = 7)
row3 <- plot_grid(plt_fig1d, labels = "D", label_size = 7)
row4 <- plot_grid(plt_fig1e,
                  plt_fig1f + thm + theme(axis.text.y = element_text(size = 7)),
                  plt_fig1g, labels = c("E", "F", "G"), nrow = 1, rel_widths = c(0.35, 0.3, 0.35), label_size = 7)
row5 <- plot_grid(plt_fig1h, labels = "H", label_size = 7)
...
cairo_pdf(file = "plts_ready_pdf/figure_1.pdf", width = conv_unit(18, "cm", "inch"), height = conv_unit(18.5, "cm", "inch"), family = "Arial")
plt_fig1
dev.off()
```
...
```{r}
# Table 1 ----------
genotypesdf  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 9", skip = n_skip)
genotypesdf %<>% 
...
dir.create("data/major_revision_3_finalplots/tbls")

genotypesdf %>% 
  reshape2::melt() %>%
  set_colnames(c("id", "locus", "value")) %>% 
  group_by(locus) %>%
  summarise(prop_under_0 = mean(value < 0)*100,
            prop_0 = mean(value == 0)*100,
            prop_above_0 = mean(value > 0)*100) %>% set_colnames(c("locus", "% under 0", "% equals 0", "% above 0")) %>% 
  writexl::write_xlsx(path = "plts_ready/tbl_1_ds2.xlsx")
```
...
freqsdf  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 1", skip = n_skip)
frequent_ncs <- freqsdf$`Bar color`[freqsdf$`Relative frequency` > 0.1] %>% set_names(freqsdf$`Nucleotide change`[freqsdf$`Relative frequency` > 0.1])
gainsdf_traj  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 10", skip = n_skip)
...
# Figure 2D ----------
modeldf  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 11", skip = n_skip)
...
# Figure 2E ----------
bgainreddf  <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 12", skip = n_skip)
plt_fig2e <- ggplot(bgainreddf) +
  ...
  theme(legend.key.width = unit(0.2, 'cm'), legend.title = element_blank(),
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(plot.tag = element_text(face = "bold"))

# assembling Figure 2 ----------
row1 <- plot_grid(plt_fig2a + labs(tag = "A"), plt_fig2b + labs(tag = "B"), label_size = 7)
row2 <- plot_grid(plt_fig2c + labs(tag = "C"), label_size = 7)
...
```{r}
# Figure 3A ----------
bindingdf <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 13", skip = n_skip)
...
# Figure 3B ----------
tcelldf <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 14", skip = n_skip)
...
# assembling Figure 3 -----------
figure_3 <- plot_grid(plt_fig3a, plt_fig3b, ncol = 1, labels = c("A", "B"), label_size = 7)
...
```{r}
# Figure 4 ----------
mdldf <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 15", skip = n_skip)

varlabels <- c("Low net binding gain", "Age > 65", "Sex: male", "High deprivation index",
               "Diabetes", "Medication for high BP or cholesterol",
               "Immune-related disorders", "Respiratory disorders", "BMI ≥ 35",
               "0-25% of the UK population vaccinated",
               "25-50% of the UK population vaccinated",
               "> 50% of the UK population vaccinated")

figure_4 <- CreateForestPlot(mdldf, varlabels = varlabels)

# exporting Figure  -----------
png(filename = "plts_ready/figure_4.png", width = 18, height = 7, units = "cm", res = 300)
figure_4
dev.off()

cairo_pdf(file = "plts_ready_pdf/figure_4.pdf", width = conv_unit(18, "cm", "inch"), height = conv_unit(10, "cm", "inch"))
figure_4
dev.off() 
```
...
bloomdf <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 16", skip = n_skip)
...
gainlossdf_bloom <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 17", skip = n_skip)
plt_supplfig1b <- CreateGainlossPlots(gainlossdf_bloom, facet_row_n = 2, pvalpos = 0.06)

# assembling Supplementary Figure 1 -----------
row1 <- cowplot::plot_grid(plt_supplfig1a, labels = c("A"), label_size = 7)
row2 <- cowplot::plot_grid(plt_supplfig1b, labels = c("B"), label_size = 7)
...
gainlossdf_iedb <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 18", skip = n_skip)
plt_supplfig2a <- CreateGainlossPlots(gainlossdf_iedb, facet_row_n = 1, pvalpos = 70) +
  xlab("") + ylab("Number of mutations")

# Supplementary Figure 2B ----------
gainlossdf_rubella <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 19", skip = n_skip)
plt_supplfig2b <- CreateGainlossPlots(gainlossdf_rubella, facet_row_n = 1, pvalpos = 10) +
  xlab("") + ylab("Number of mutations")

# assembling Supplementary Figure 2 -----------
plt_supplfig_2 <- cowplot::plot_grid(plt_supplfig2a, plt_supplfig2b,ncol = 1, labels = c("A", "B"), label_size = 7)
...
```{r}
gainlossdfs_context <- list(upstream_A = readxl::read_excel("supplementary_data_tables/Source_Data.xlsx",
                                                            sheet = "Source Data Table 20", skip = n_skip),
                            upstream_C = readxl::read_excel("supplementary_data_tables/Source_Data.xlsx",
                                                            sheet = "Source Data Table 21", skip = n_skip),
                            upstream_G = readxl::read_excel("supplementary_data_tables/Source_Data.xlsx",
                                                            sheet = "Source Data Table 22", skip = n_skip),
                            upstream_U = readxl::read_excel("supplementary_data_tables/Source_Data.xlsx",
                                                            sheet = "Source Data Table 23", skip = n_skip))
...
```{r}
netgainsdf <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 24", skip = n_skip)
...
aachfreqsdf_GU <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 25", skip = n_skip)
...
hydrophchdf_GU <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 26", skip = n_skip)
...
gainlossdf_GU <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 27", skip = n_skip)
...
cowplot::plot_grid(cowplot::plot_grid(plt_supplfig5a, plt_supplfig5b, nrow = 1,
                                      labels = c("A", "B"), rel_widths = c(0.6, 0.4), label_size = 7), 
                   plt_supplfig5c, ncol = 1, labels = c(NA, "C"), label_size = 7)
dev.off()
```

# Supplementary Figure 6
```{r}
bgainsdf_alleles <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 28", skip = n_skip)
bgainsdf_alleles %<>%
...
```{r}
bgainsdf_invitro_subj <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 29", skip = n_skip)
...
```{r}
# plotting -----------
varlabels_risk <- c("Low net binding gain", "Age > 65", "Sex: male", "High deprivation index",
                    "Diabetes", "Medication for high BP or cholesterol",
                    "Immune-related disorders", "Respiratory disorders", "BMI ≥ 35",
                    "0-25% of the UK population vaccinated",
                    "25-50% of the UK population vaccinated",
                    "> 50% of the UK population vaccinated",
                    "Carrying B*53", "Carrying A*11")

varlabels_prot <- c("Low net binding gain", "Age > 65", "Sex: male", "High deprivation index",
                    "Diabetes", "Medication for high BP or cholesterol",
                    "Immune-related disorders", "Respiratory disorders", "BMI ≥ 35",
                    "0-25% of the UK population vaccinated",
                    "25-50% of the UK population vaccinated",
                    "> 50% of the UK population vaccinated",
                    "Carrying A*03", "Carrying B*40:02", "Carrying A*01")

mdldf_risk <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 30", skip = n_skip)
mdldf_prot <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 31", skip = n_skip)

plt_forest_risk <- CreateForestPlot(mdldf_risk, varlabels = varlabels_risk)
plt_forest_prot <- CreateForestPlot(mdldf_prot, varlabels = varlabels_prot)

# exporting Figure  -----------
png(filename = "plts_ready/supplfig_9.png", width = 18, height = 18, units = "cm", res = 300)
cowplot::plot_grid(plt_forest_risk, plt_forest_prot, labels = c("A", "B"), label_size = 7, ncol = 1)
dev.off()
```
...
```{r}
# loading data -----------
fitnessdf <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 32", skip = n_skip)

# checking tendencies between binding gains vs. delta values -----------
netgain_params <- colnames(fitnessdf)[4:9]
corresdf <- map_df(netgain_params, \(.x) {
                       Hmisc::rcorr(fitnessdf[[.x]], fitnessdf[["Fitness change"]], type = "spearman") %>% ExtractCorrResults(string_output = FALSE)
                     }) %>%
  mutate(prettyp = PrettyPvalues(P)) %>% 
  mutate(r_round = round(r, digits = 2))

corresdf$pos <- map_dbl(netgain_params, \(.x) {
  extreme_vals <- fitnessdf[[.x]] %>% Rfast::min_max()
  quantile(extreme_vals[1]:extreme_vals[2], probs = 0.5)
})

corresdf$pretty_str <-  corresdf$prettyp %>%
  gsub("paste\\(", "", .) %>%
  paste0('paste(ρ, " = ", ', corresdf$r_round, ', ", ", ' , .)
...
pltslist <- pmap(list(pltslist, corresdf$pretty_str, corresdf$pos),
                     \(pltobj = ..1, corrlab = ..2, corrpos = ..3) {pltobj + annotate("text", x = corrpos, y = 3, label = corrlab, parse = TRUE, size = 7/.pt) + thm})

plt_supplfig10 <- pltslist %>% cowplot::plot_grid(plotlist = ., ncol = 2, labels = c("A", "B", "C", "D", "E", "F"), label_size = 7)
...
# Figure R1
The following figure was included in the last response letter, showing the impact of downsampling on binding gains/losses per nucleotide change types.

```{r}
# Figure R1 (from the response letter to the reviewers) ----------
gainlossdf_figr1 <- readxl::read_excel("supplementary_data_tables/Source_Data.xlsx", sheet = "Source Data Table 33", skip = n_skip)
plt_figr1 <- CreateGainlossPlots(gainlossdf_figr1, facet_row_n = 1, pvalpos = 15)

png(filename = "plts_ready/figure_r_1.png", width = 30, height = 10, units = "cm", res = 300)
plt_figr1
dev.off()
```
