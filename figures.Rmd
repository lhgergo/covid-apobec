...
plt_fig2d <- ggplot(modeldf_toplot) +
  ...
  geom_text(aes(x = 0.005, y = pos_y, hjust = 0, label = pval_pretty), parse = T, size = 3) +
...
plt_fig3a <- ggroc(rocobj) +
  ...
  theme_classic() + xlab("Specificity") + ylab("Sensitivity")
...
plt_fig3b <- ggplot(tcelldf_long) +
  ...
  scale_colour_brewer(type = "div", palette = "Set1") +
  annotate("text", x = 1.5, y = tcelldf$`CD25+ fraction - peptide pool 1` %>% max %>% add(1),
           label = PrettyPvalues(pval_fc_unstim_orig),
           hjust = 0.5, parse = TRUE) +
  annotate("text", x = 2.5, y = tcelldf$`CD25+ fraction - peptide pool 2` %>% max %>% add(1.1),
...
# assembling Figure 3 -----------
figure_3 <- plot_grid(plt_fig3a, plt_fig3b, nrow = 1, labels = c("A", "B"))
...
plt_supplfig1b <- CreateGainlossPlots(gainlossdf_bloom, facet_row_n = 1, pvalpos = 0.06)
...
png(filename = "plts_ready/supplfig_1.png", width = 18.46, height = 20, units = "cm", res = 300)
supplfig_1
dev.off()
```
...
png(filename = "plts_ready/supplfig_2.png", width = 18.46, height = 20, units = "cm", res = 300)
plt_supplfig_2
dev.off()
```
...
png(filename = "plts_ready/supplfig_3.png", width = 18.46, height = 20.1, units = "cm", res = 300)
plt_supplfig3
dev.off()
```
...
# exporting Supplementary Figure 3 -----------
png(filename = "plts_ready/supplfig_4.png", width = 18.46, height = 20, units = "cm", res = 300)
plt_supplfig4
dev.off()
```
...
png(filename = "plts_ready/supplfig_6.png", width = 18.46, height = 10, units = "cm", res = 300)
plt_supplfig6
dev.off()
```
